...
import AutoPartsDetailDataContext from '../../detailsContext'
import ProductDetail from 'modules/products/module/productDetail'
import { useRouter } from 'next/router'
import { useLoading } from 'hooks/useLoading'

const FormData = require('form-data')
const AutoPartDetail: FC = () => {
  const [product, setProduct] = useState<IProductDetail>()
  const route = useRouter()
  const { setLoading } = useLoading()

  const fetchProduct = useCallback(async () => {
    if (!route.query.autoPartID) return
    setLoading(true)

    const productService = new ProductService()
    const formData = new FormData()
    formData.append('PID', route.query.autoPartID)
  ...
  }, [route.query.autoPartID])
  useEffect(() => {
    fetchProduct()
  }, [fetchProduct])
  return (
    <AutoPartsDetailDataContext productDetail={product}>
      <ProductDetail />
    </AutoPartsDetailDataContext>
  )
}
...
//   const paths = data?.map(({ p_ID }) => ({ params: { autoPartID: `${p_ID}` } }))
...
//   const productService = new ProductService()
//   const formData = new FormData()
//   formData.append('PID', params.params.autoPartID)
